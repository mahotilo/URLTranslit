$(function(){
	...
	function myTransliterate(text) {
	  return text.split('').map(function (char) { 
		return TransPairs[char] || char; 
	  }).join("");
	}
	...
	TranslitSlug = function(e){
		...
		.replace(/\-([+_])\-/g, '$1');		
		$new_title_inp.val(slug);
	}	


	//add control item with onclick
	var addURLTranslit = function(){
		$('#gp_rename_form .label_synchronize').first().append('<a class="slug_transliterate" onclick="TranslitSlug(event);">Transliterate</a>')
	};


	//catch rename form creation
	$( document ).ajaxSuccess(function( event, xhr, settings ) {
	  if ( settings.url.includes('cmd=renameform') ) {
	  	addURLTranslit();
	  }
	});
